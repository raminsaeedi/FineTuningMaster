...
This chapter defines the dashboard-design concepts, visualization principles, language-model mechanisms, QLoRA adaptation, retrieval augmentation, and structured-output terminology required by the study. Detailed implementation choices and alternatives are reserved for Chapter 5.
...
The project implements this design problem as structured generation. Its input is a dashboard brief with users, goals, KPIs, data columns, and constraints. Its target is a typed recommendation with the fields \texttt{context\_summary}, \texttt{kpi\_chart\_mapping}, \texttt{layout}, \texttt{styling}, \texttt{interactions}, and \texttt{rationales}. This structure is a project-specific design contract. It is not presented as a universal definition of a dashboard. It makes the target explicit and allows the methods in the thesis to be compared on the same output interface.

\subsection{Dashboards versus Individual Visualizations}
...
\subsection{Users, Goals, KPIs, and Data Context}
...
\subsection{Expressiveness and Effectiveness}
...
\subsection{Visual Hierarchy and Information Density}
...
\subsection{Readability, Labels, and Scales}
...
\subsection{Color and Accessibility}
...
\section{Analytical Tasks and Chart Types}
...
\subsection{Composition and Part-to-Whole}
...
\subsection{Flow and Network Relationships}
...
\subsection{Limitations and Hallucination}
...
where \(A \in \mathbb{R}^{r \times d_{\mathrm{in}}}\) and \(B \in \mathbb{R}^{d_{\mathrm{out}} \times r}\) are trainable matrices, \(r\) is the adapter rank, and \(\alpha\) is a scaling factor. The rank \(r\) is chosen to be much smaller than the dimensions of the original matrix. During training, \(W_0\) is frozen and only \(A\) and \(B\) are updated.
...
\subsection{Why PEFT for This Thesis}
...
\iffalse
\subsubsection{Other PEFT and Memory-Efficient Fine-Tuning Algorithms}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.2000}}@{}}
\caption{Candidate adaptation algorithms and the reason each was or was not selected as the primary method.}\tabularnewline
\toprule\noalign{}
...
Separable artifact
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Memory profile of the frozen base
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Fit to the four requirements
\end{minipage} \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
...
\textbf{QLoRA} \cite{dettmers2023qlora} & \textbf{Low-rank update of selected linear layers over a 4-bit quantized frozen base} & \textbf{Yes} & \textbf{4-bit NF4 with double quantization} & \textbf{Meets all four requirements} \\
\end{longtable}
\endgroup
\end{landscape}
...
\fi
\subsection{Why QLoRA for This Thesis}
...
\subsection{RAG Limitations and Grounding}
...
\section{Structured LLM Outputs}
...
\subsection{JSON Schema and Schema-Constrained Generation}
...
\subsection{Parsing, Validation, and Semantic Correctness}
