"""Tests for Pilot v6 quality-constrained, unbalanced sampling."""

import json
import sqlite3
from pathlib import Path

from src.core.schemas import ChartType, DashboardBrief, DesignOutput, GoldItem, KPIChartMapping, TaskType
from src.data_pipeline.builders.nvbench_builder import NvBenchBuilder
from src.data_pipeline.nvbench_pilot import _record
from src.data_pipeline.nvbench_pilot_v6 import (
    NORMALIZED_CHART_TYPES,
    before_after_v5_v6,
    compute_target_distribution,
    select_pilot_v6,
)
from src.data_pipeline.nvbench_profile import DbProfiler
from src.data_pipeline.nvbench_source import DbMetadataResolver, load_mapping

_REPO_ROOT = Path(__file__).resolve().parents[2]
_MAPPING_PATH = _REPO_ROOT / "src" / "config" / "data" / "nvbench_mapping.yaml"

_CFG = {
    "identifier": {"strong_unique_ratio": 0.98, "strong_min_distinct": 20, "ambiguous_unique_ratio": 0.5,
                   "name_patterns": ["(^|_)id$", "^id(_|$)", "identifier", "(^|_)key$", "(^|_)code$"]},
    "chart": {"pie": {"max_categories": 8}, "scatter": {"min_distinct_values": 10},
              "stacked_bar": {"max_group_cardinality": 8}},
    "scoring": {"weights": {"source_fidelity": 30, "kpi_validity": 20, "chart_suitability": 25,
                            "constraint_completeness": 15, "db_profile_support": 10}, "tier_a_min_score": 90},
    "sampling": {"seed": 42, "target_per_chart": 20, "db_cap": 10, "near_dup_threshold": 0.8},
    "time_grain": {"supported": ["YEAR", "MONTH", "DAY", "WEEKDAY", "WEEK", "QUARTER", "HOUR", "DATE"],
                   "name_hints": ["date", "time", "year", "month", "day", "week", "quarter", "hour"],
                   "temporal_charts": ["line", "bar"]},
}

_WORDBANK = ("alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike "
             "november oscar papa quebec romeo sierra tango uniform victor whiskey xray yankee zulu").split()


def _distinct_goal(chart, i):
    offset = sum(ord(c) for c in chart)
    words = " ".join(_WORDBANK[(offset + i * 7 + j * 5) % len(_WORDBANK)] for j in range(8))
    return f"{chart} {words} scenario {i} cohort {offset + i}"


def _gold_item(item_id, chart_type, group_id, goal, db_id):
    kpi = f"k_{item_id}"
    brief = DashboardBrief(item_id=item_id, users=f"user {item_id}", goals=[goal], kpis=[kpi],
                           columns=[{"name": kpi, "dtype": "number", "role": "measure"}],
                           extra={"provenance": {"source_group_id": group_id, "db_id": db_id}})
    rec = DesignOutput(context_summary={},
                       kpi_chart_mapping=[KPIChartMapping(kpi=kpi, task_type=TaskType.COMPARISON,
                                                          chart_type=ChartType(chart_type), encoding={})],
                       layout={}, styling={}, interactions=[], rationales=[])
    return GoldItem(item_id=item_id, brief=brief, recommendation=rec, split="train")


def _pool(per_chart_map):
    items = []
    for chart, n in per_chart_map.items():
        for i in range(n):
            items.append(_gold_item(f"{chart}:{i}", chart, f"grp:{chart}:{i}",
                                     _distinct_goal(chart, i), db_id=f"db_{chart}_{i}"))
    return items


# --------------------------------------------------------------------------- #
# compute_target_distribution
# --------------------------------------------------------------------------- #
def test_target_distribution_reserves_scatter_and_splits_remainder():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 10, "stacked_bar": 30})
    target = compute_target_distribution(items, seed=42, total=100, reserved_chart="scatter", reserved_cap=10)
    assert target["scatter"] == 10
    assert sum(target.values()) == 100
    # remainder 90 over 4 charts = 22 or 23 each
    assert set(target[c] for c in ("bar", "line", "pie", "stacked_bar")) <= {22, 23}


def test_target_distribution_caps_at_reserved_cap_even_with_more_available():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 50, "stacked_bar": 30})
    target = compute_target_distribution(items, seed=42, total=100, reserved_chart="scatter", reserved_cap=10)
    assert target["scatter"] == 10  # never exceeds the cap even when more are available


def test_target_distribution_never_exceeds_true_availability():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 4, "stacked_bar": 30})
    target = compute_target_distribution(items, seed=42, total=100, reserved_chart="scatter", reserved_cap=10)
    assert target["scatter"] == 4  # capped by real availability, not the nominal cap
    assert sum(target.values()) == 96 + 4  # remainder still redistributed to reach total where possible


def test_target_distribution_deterministic():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 10, "stacked_bar": 30})
    t1 = compute_target_distribution(items, seed=42, total=100)
    t2 = compute_target_distribution(items, seed=42, total=100)
    assert t1 == t2


# --------------------------------------------------------------------------- #
# select_pilot_v6
# --------------------------------------------------------------------------- #
def test_select_pilot_v6_matches_target_exactly():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 15, "stacked_bar": 30})
    target = {"bar": 23, "line": 23, "pie": 22, "scatter": 10, "stacked_bar": 22}
    selected, report = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=10)
    assert selected is not None
    assert report["status"] == "ok"
    assert len(selected) == 100
    import collections
    counts = collections.Counter(it.recommendation.kpi_chart_mapping[0].chart_type.value for it in selected)
    assert dict(counts) == target


def test_select_pilot_v6_unique_source_groups():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 15, "stacked_bar": 30})
    target = {"bar": 23, "line": 23, "pie": 22, "scatter": 10, "stacked_bar": 22}
    selected, report = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=10)
    groups = {it.brief.extra["provenance"]["source_group_id"] for it in selected}
    assert len(groups) == 100


def test_select_pilot_v6_deterministic():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 15, "stacked_bar": 30})
    target = {"bar": 23, "line": 23, "pie": 22, "scatter": 10, "stacked_bar": 22}
    s1, _ = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=10)
    s2, _ = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=10)
    assert [it.item_id for it in s1] == [it.item_id for it in s2]


def test_select_pilot_v6_insufficient_below_100_fails():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 6, "stacked_bar": 30})  # only 6 scatter, target 10
    target = {"bar": 23, "line": 23, "pie": 22, "scatter": 10, "stacked_bar": 22}
    selected, report = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=10)
    assert selected is None
    assert report["status"] == "insufficient_tier_a_candidates_after_dedup"
    assert "scatter" in report["short_charts"]


def test_select_pilot_v6_reports_availability_and_dedup_stats():
    items = _pool({"bar": 30, "line": 30, "pie": 30, "scatter": 15, "stacked_bar": 30})
    target = {"bar": 23, "line": 23, "pie": 22, "scatter": 10, "stacked_bar": 22}
    _selected, report = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=10)
    assert report["availability_per_chart_raw"]["scatter"] == 15
    assert report["unique_groups_per_chart"]["scatter"] == 15  # 1 item per group in this fixture
    assert "dropped_exact_goal_duplicates_by_chart" in report
    assert "near_dup_removed_by_chart" in report


def test_select_pilot_v6_no_db_cap_fallback_silently_accepted():
    # All scatter items share one database; db_cap smaller than scatter target
    # forces a would-be fallback -- v6 must refuse, not silently over-admit.
    items = [_gold_item(f"scatter:{i}", "scatter", f"grp:scatter:{i}", _distinct_goal("scatter", i), db_id="shared")
             for i in range(15)]
    for chart in ("bar", "line", "pie", "stacked_bar"):
        items += [_gold_item(f"{chart}:{i}", chart, f"grp:{chart}:{i}", _distinct_goal(chart, i), db_id=f"db_{chart}_{i}")
                  for i in range(23)]
    target = {"bar": 23, "line": 23, "pie": 22, "scatter": 10, "stacked_bar": 22}
    selected, report = select_pilot_v6(items, seed=42, target_per_chart=target, db_cap=2)
    assert selected is None
    assert report["status"] == "insufficient_tier_a_candidates_within_db_cap"


# --------------------------------------------------------------------------- #
# before_after_v5_v6
# --------------------------------------------------------------------------- #
_EMP_DDL = """
CREATE TABLE employee (Employee_ID INTEGER PRIMARY KEY, salary REAL, dept TEXT);
INSERT INTO employee VALUES (1, 50000, 'eng'); INSERT INTO employee VALUES (2, 60000, 'eng');
"""


def _entry(chart, nl, db_id, x, y, sql):
    return {"chart": chart, "db_id": db_id, "hardness": "Easy",
            "vis_query": {"vis_part": f"Visualize {chart.upper()}", "data_part": {"sql_part": sql, "binning": ""}, "VQL": sql},
            "vis_obj": {"chart": chart.lower(), "x_name": x, "y_name": y, "x_data": [[1]], "y_data": [[1]],
                        "classify": [], "describe": ""},
            "nl_queries": nl}


def test_before_after_v5_v6_corpus_level(tmp_path):
    good = _entry("Bar", ["total salary by dept"], "b1", "dept", "SUM(salary)",
                  "SELECT dept , SUM(salary) FROM employee GROUP BY dept")
    nv = tmp_path / "NVBench.json"
    nv.write_text(json.dumps({"1": good}), encoding="utf-8")
    cache = tmp_path / "databases"; (cache / "b1").mkdir(parents=True)
    con = sqlite3.connect(cache / "b1" / "b1.sqlite"); con.executescript(_EMP_DDL); con.commit(); con.close()
    result = NvBenchBuilder(str(nv), cache_root=str(cache), mapping_path=str(_MAPPING_PATH)).build()
    records = [_record(it) for it in result.accepted]
    mapping = load_mapping(str(_MAPPING_PATH))
    resolver = DbMetadataResolver(str(cache)); profiler = DbProfiler(resolver)
    ba = before_after_v5_v6(records, records, mapping, resolver, profiler, _CFG)
    assert ba["v5"]["n"] == 1 and ba["v6"]["n"] == 1
    assert "item_ids" not in json.dumps(ba)
