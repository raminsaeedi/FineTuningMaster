...
The specification, recorded here so that it can be executed without redesign, is as follows. The instrument covers six dimensions, each on a 1–5 scale with anchors defined at 1, 3 and 5: chart appropriateness, layout quality, styling and accessibility, interaction design, rationale quality, and overall usefulness. The item pool is the fixed 40-row selection drawn from the held-out test split and frozen with the dataset. The intended design is 40 items × 4 methods × 3 independent ratings per output, giving 160 rating units and 480 ratings distributed as a balanced 80 ratings per rater across six raters; any smaller configuration is recorded as a pilot and is not pooled with a final study. Raters see the dashboard brief, one anonymised recommendation, the six scales and an optional comment field; method identity, model identity, seed, automatic metrics and the reference recommendation are hidden. Each study is isolated by dataset, model and seed, and its manifest records the source prediction paths and digests, run identifiers, configuration hashes, dataset and knowledge-base digests, the item identifiers, the rubric hash and the expected counts, so that an analysis refuses to run if the underlying predictions changed after the study was created.
