"""Build and validate Pilot v6: quality-constrained, unbalanced 100-record pilot.

Reuses the v5 quality gate unchanged (KPI/SQL aggregation agreement, required
time-grain/grouping preservation, identifier/chart suitability -- nothing
reverted or relaxed). Only the sampling *balance* contract changes: since only
~10 unique high-confidence scatter groups exist in the whole corpus, the target
distribution reserves up to 10 for scatter (derived from real data, never
hardcoded) and spreads the remaining 90 evenly across bar/line/pie/stacked_bar,
so the pilot still totals exactly 100 Tier-A records with zero Tier-B fallback.

Writes only under ``--out``. Refuses any ``frozen`` path. Reads v5's staging
output read-only for the before/after comparison; v1-v5 are never modified.

Usage:
    python experiments/scripts/run_nvbench_pilot_v6.py \
        --out data/staging/dashboard_v3/nvbench_pilot_v6 --seed 42 --db-cap 10
"""
...
from src.data_pipeline.nvbench_pilot_v6 import (  # noqa: E402
...
DEFAULT_V5_TIER_A = "data/staging/dashboard_v3/nvbench_pilot_v5/quality_pool/tier_a_candidates.jsonl"
...
_MANDATORY_QUALITY_RULES = (
    "kpi_sql_aggregation_conflict", "mixed_aggregate_ambiguous_kpi", "query_aggregation_conflict",
    "missing_required_time_grain", "missing_required_grouping",
    "identifier_as_measure", "identifier_as_continuous_kpi", "meaningless_identifier_aggregation",
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build and validate Pilot v6 (quality-constrained, unbalanced).")
    p.add_argument("--out", default="data/staging/dashboard_v3/nvbench_pilot_v6")
    ...
    p.add_argument("--v5-tier-a", default=DEFAULT_V5_TIER_A)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--total", type=int, default=100)
    p.add_argument("--db-cap", type=int, default=10)
    p.add_argument("--reserved-chart", default="scatter")
    p.add_argument("--reserved-cap", type=int, default=10)
    return p.parse_args()
...
def _write_audit_template(records, quality_by_id: dict, path: Path) -> None:
    with path.open("w", encoding="utf-8", newline="") as f:
        ...
        for r in records:
            ...
            row = {
                "item_id": r.get("item_id", ""), "split": r.get("split", ""), "db_id": prov.get("db_id", ""),
                ...
                "encoding_x": enc.get("x", ""), "encoding_y": enc.get("y", ""),
                ...
                "quality_tier": q.get("tier", ""), "quality_score": q.get("quality_score", ""),
...
def _write_validation_md(report: dict, path: Path) -> None:
    L = ["# Pilot v6 — Validation Report", ""]
    L.append(f"- pilot: `{report['pilot_dir']}`")
    L.append(f"- built_at: {report['built_at']}")
    L.append(f"- status: **{report['status']}**")
    L.append(f"- selected: {report['counts'].get('selected')}  target_distribution: {report.get('target_distribution')}")
    ...
    for section in ("structural", "semantic", "duplicate", "leakage", "quality"):
        if section not in report.get("checks", {}):
            continue
        L.append("")
...
def _write_quality_md(summary: dict, sampling_report: dict, path: Path) -> None:
    L = ["# Pilot v6 — Quality Pool Report", ""]
    ...
    L.append("")
    L.append("## Sampling — target distribution and availability")
    L.append(f"- target_per_chart: {sampling_report.get('target_per_chart')}")
    L.append(f"- availability_per_chart_raw (Tier-A, before one-per-group): {sampling_report.get('availability_per_chart_raw')}")
    L.append(f"- unique_groups_per_chart (after one-per-group): {sampling_report.get('unique_groups_per_chart')}")
    L.append(f"- dropped_exact_goal_duplicates_by_chart: {sampling_report.get('dropped_exact_goal_duplicates_by_chart')}")
    L.append(f"- near_dup_removed_by_chart: {sampling_report.get('near_dup_removed_by_chart')}")
    L.append(f"- final chart_counts: {sampling_report.get('chart_counts')}")
    L.append("")
    L.append("## Tier by chart (full pool)")
    for chart, counts in sorted(summary["tier_by_chart"].items()):
        L.append(f"- `{chart}`: {counts}")
    L.append("")
    L.append("## Rule failure counts (full pool)")
...
def _write_before_after_md(ba: dict, path: Path) -> None:
    L = ["# Pilot v5 vs Pilot v6 — Before/After (corpus-level, unchanged v5 quality gate)", ""]
    if "error" in ba:
        L.append(f"- {ba['error']}")
        path.write_text("\n".join(L), encoding="utf-8")
        return
    ...
    L.append("| metric | v5 | v6 |")
    L.append("|---|---|---|")
    for mkey in metrics:
        L.append(f"| {mkey} | {ba['v5'].get(mkey)} | {ba['v6'].get(mkey)} |")
    path.write_text("\n".join(L), encoding="utf-8")
...
def main() -> None:
    ...
    target_distribution = compute_target_distribution(
        pool["tier_a"], seed=args.seed, total=args.total,
        reserved_chart=args.reserved_chart, reserved_cap=args.reserved_cap,
    )
    selected, sampling_report = select_pilot_v6(
        pool["tier_a"], seed=args.seed, target_per_chart=target_distribution, db_cap=args.db_cap,
    )
    ...
    _write_quality_md(pool["summary"], sampling_report, reports_dir / "quality_report.md")
    _write_quality_rule_failures_csv(pool["summary"]["rule_failure_counts"], reports_dir / "quality_rule_failures.csv")

    built_at = datetime.now(timezone.utc).isoformat()

    v5_tier_a_path = _resolve(args.v5_tier_a)
    v6_corpus_for_ba = ([_record(it) for it in selected] if selected is not None
                        else [_record(it) for it in pool["tier_a"]])
    if v5_tier_a_path.exists():
        v5_records, _ = read_jsonl_strict(v5_tier_a_path)
        before_after = before_after_v5_v6(v5_records, v6_corpus_for_ba, mapping, resolver, profiler, cfg)
    else:
        before_after = {"error": f"v5 Tier-A pool not found at {v5_tier_a_path}"}
    write_json(before_after, reports_dir / "before_after_v5_v6.json")
    _write_before_after_md(before_after, reports_dir / "before_after_v5_v6.md")
    ...
    manifest_meta = {
        "selection_strategy": "quality_constrained_unbalanced",
        "reason": (
            f"Only {sampling_report['availability_per_chart_raw'].get(args.reserved_chart, 0)} Tier-A "
            f"{args.reserved_chart} candidates ({sampling_report['unique_groups_per_chart'].get(args.reserved_chart, 0)} "
            f"unique source groups) remained after quality filtering and deduplication."
        ),
        "quality_priority": True,
        "tier_b_fallback_used": False,
        "target_distribution": target_distribution,
        "availability_per_chart_raw": sampling_report["availability_per_chart_raw"],
        "unique_groups_per_chart": sampling_report["unique_groups_per_chart"],
        "dropped_exact_goal_duplicates_by_chart": sampling_report["dropped_exact_goal_duplicates_by_chart"],
        "near_dup_removed_by_chart": sampling_report["near_dup_removed_by_chart"],
    }

    if selected is None:
        report = {
            "pilot_dir": out_dir.as_posix(), "built_at": built_at, "passed": False,
            "status": sampling_report["status"], "target_distribution": target_distribution,
            "counts": {**common_counts, "selected": None},
            "quality_rule_version": cfg.get("rule_version"),
            "checks": {}, "failed_mandatory": [{
                "check": "quality_constrained_target_met", "passed": False, "severity": "mandatory",
                "n": len(sampling_report.get("short_charts", [])), "item_ids": [],
                "detail": f"short charts {sampling_report.get('short_charts')}: chart_counts "
                          f"{sampling_report.get('chart_counts')} vs target {target_distribution}",
            }],
            "distributions": None,
        }
        ...
        write_json({
            ...
            "status": sampling_report["status"], "passed": False, "seed": args.seed,
            **manifest_meta,
            "selection_policy": {"db_cap": args.db_cap, "near_duplicate_threshold": NEAR_DUP_THRESHOLD},
        ...
        if stale_acc.exists():
            stale_acc.unlink()
        print(f"[FAIL] {sampling_report['status']}: chart_counts={sampling_report.get('chart_counts')} "
    ...
    if acc_path.exists() and acc_path.read_bytes() == acc_bytes:
        ...
        print("[conflict] existing Pilot v6 differs from a fresh rebuild; not overwriting.")
        raise SystemExit(1)
    else:
        acc_path.write_bytes(acc_bytes)
        print(f"[build] wrote {len(records)} Pilot v6 records to {out_dir}")

    # Deterministic rebuild check.
    _, pool2 = _build_pool()
    target2 = compute_target_distribution(pool2["tier_a"], seed=args.seed, total=args.total,
                                          reserved_chart=args.reserved_chart, reserved_cap=args.reserved_cap)
    selected2, _ = select_pilot_v6(pool2["tier_a"], seed=args.seed, target_per_chart=target2, db_cap=args.db_cap)
    ...
    kpi_sql = _ids_where(lambda q: any(r in q.get("failed_rules", [])
                                       for r in ("kpi_sql_aggregation_conflict", "mixed_aggregate_ambiguous_kpi")))
    ...
    id_measure = _ids_where(lambda q: any(r.startswith("identifier_") or r == "meaningless_identifier_aggregation"
                                          for r in q.get("failed_rules", [])))
    bad_scatter_axis = _ids_where(lambda q: any(r in q.get("failed_rules", []) for r in _SCATTER_AXIS_RULES))

    import collections as _c
    chart_counts_final = _c.Counter((_mapping0(r) or {}).get("chart_type", "?") for r in rb_records)

    def mk(name, ids, detail):
        return {"check": name, "passed": not ids, "severity": "mandatory", "n": len(ids),
                "item_ids": ids, "detail": detail}

    dist_ok = all(chart_counts_final.get(c, 0) == n for c, n in target_distribution.items())
    quality_checks = [
        mk("exactly_100_records", [] if len(rb_records) == expected_total else ["count"],
           f"{len(rb_records)} records (expected {expected_total})"),
        {"check": "matches_manifest_target_distribution", "passed": dist_ok, "severity": "mandatory",
         "n": 0 if dist_ok else 1, "item_ids": [],
         "detail": f"chart counts {dict(chart_counts_final)} vs target {target_distribution}"},
        ...
        mk("zero_kpi_sql_conflicts", kpi_sql, "encoded/KPI aggregate conflicts with source SQL"),
        mk("zero_missing_time_grain", miss_grain, "source time grain missing from record"),
        mk("zero_missing_grouping", miss_group, "source grouping missing from record"),
        mk("zero_identifier_as_measure", id_measure, "identifier used as a meaningless measure"),
        mk("zero_invalid_scatter_axes", bad_scatter_axis, "identifier/categorical scatter axis"),
    ]
