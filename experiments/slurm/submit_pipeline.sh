...
# ── Defaults ────────────────────────────────────────────────────────────────
BASE_CONFIG="src/config/base.yaml"
MODEL_CONFIG=""
EXPERIMENT_CONFIG=""
PARTITION="gpu"
EXTRA_OVERRIDES=""
...
EXPERIMENT_DIR="${PROJECT_ROOT}/experiments/outputs/experiments/${EXPERIMENT_ID}"
...
if [[ "$SKIP_DATA" == "false" ]]; then
    ...
    DATA_JID=$(submit_job "experiments/slurm/templates/prepare_data.slurm" "")
    echo "job $DATA_JID"
    PREV_JID="$DATA_JID"
fi

echo -n "Stage 2 — train           : "
TRAIN_JID=$(submit_job "experiments/slurm/templates/train.slurm" "$PREV_JID")
echo "job $TRAIN_JID"
PREV_JID="$TRAIN_JID"

echo -n "Stage 3 — inference       : "
INFER_JID=$(submit_job "experiments/slurm/templates/inference.slurm" "$PREV_JID")
echo "job $INFER_JID"
PREV_JID="$INFER_JID"

echo -n "Stage 4 — evaluate        : "
EVAL_JID=$(submit_job "experiments/slurm/templates/evaluate.slurm" "$PREV_JID")
echo "job $EVAL_JID"

echo "========================================================"
echo "  Logs   : ${EXPERIMENT_DIR}/logs/"
echo "  Compare: python pipeline/compare.py --outputs-root experiments/outputs/experiments"
echo "========================================================"
