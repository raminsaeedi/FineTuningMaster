...
```
src/
  config/           Hydra config groups (model, method, training, data, eval, experiment)
...
Every run writes a self-describing folder under `experiments/outputs/experiments/<id>/`:
...
```bash
python experiments/scripts/build_data.py
```
...
```
experiments/outputs/experiments/E03_qwen0_5b_ft_42/
...
```bash
python experiments/scripts/run_experiment.py --experiment E01_qwen0_5b_prompt
```

**Method B — RAG** (retrieves guideline chunks, no adapter needed):

```bash
python experiments/scripts/run_experiment.py --experiment E02_qwen0_5b_rag
```

**Method C — fine-tuned:** drop the adapter folder sent by the professor at
`experiments/outputs/experiments/E03_qwen0_5b_ft_42/adapter`, then:

```bash
python experiments/scripts/run_experiment.py --experiment E03_qwen0_5b_ft
```
...
```bash
python experiments/scripts/eval_stats.py --experiments E01_qwen0_5b_prompt E03_qwen0_5b_ft
```
...
|---|---|
| `python experiments/scripts/build_data.py` | Build `data/processed/{train,val,test}.jsonl` with hash splits |
| `python experiments/scripts/build_kb.py` | Build the RAG knowledge base (`data/knowledge_base/chunks.jsonl`) |
| `python experiments/scripts/build_perturbations.py` | Build paraphrase / missing-info test variants (enables the robustness metrics) |
| `python experiments/scripts/train.py --experiment E03_qwen0_5b_ft` | Fine-tune (GPU); writes the run folder |
| `python experiments/scripts/run_all.py --experiments E01_qwen0_5b_prompt E03_qwen0_5b_ft --seeds 42 43 44` | Run methods × seeds |
| `python experiments/scripts/infer.py --experiment E01_qwen0_5b_prompt` | Cached inference only |
| `python experiments/scripts/eval_auto.py --experiment E01_qwen0_5b_prompt` | Automatic metrics |
| `python experiments/scripts/run_experiment.py --experiment E01_qwen0_5b_prompt` | Infer + eval |
