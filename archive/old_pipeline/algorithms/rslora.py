...
Paper: Kalajdzievski, 2024 — https://arxiv.org/abs/2312.03732
Status: IMPLEMENTED

Key technique: Changes the LoRA scaling factor from alpha/r to alpha/sqrt(r).
This rank-stabilized scaling enables stable training at higher ranks (r=32, 64,
128) without divergence, while standard LoRA degrades at high ranks.

Implementation: identical to QLoRA with use_rslora=True in LoraConfig.
The experiment config sets r=32 (vs 16 in QLoRA) to exploit the stability gain.
Requires: peft >= 0.8.0
"""
...
class RSLoRAFineTuner(BaseFineTuner):
    ...
    def setup(self) -> None:
        """Load base model with 4-bit quantization and apply RSLoRA adapters."""
        import torch
        ...
        if model_cfg.get("load_in_4bit", True):
            logger.info("Configuring 4-bit NF4 quantization (RSLoRA)")
        ...
        # ── RSLoRA adapters ───────────────────────────────────────────────────
        # use_rslora=True changes the scaling from alpha/r to alpha/sqrt(r),
        # stabilising training at higher ranks (Kalajdzievski, 2024).
        lora_config = LoraConfig(
            r=lora_cfg["r"],                       # typically 32 for RSLoRA
    ...
    def save(self) -> None:
        """Save RSLoRA adapter weights and tokenizer."""
        self._adapter_dir.mkdir(parents=True, exist_ok=True)
        logger.info(f"Saving RSLoRA adapter to {self._adapter_dir}")
